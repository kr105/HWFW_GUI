  ...
  case WM_COMMAND:
  {
    ...
    if (lParam != 0)
    {
      ...
      case IDC_BTN_EXP:
        if (lpDlgParam->dtType == DT_EDIT)
        {
          ...
          if (GetSaveFilePath(hDlg, wsTmp, MAX_PATH))
          {
            ...
            if (nResult != 0)
            {
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to get project data size, error code:[%d]!", nResult);
              break;
            }
            ...
            if (nResult != 0)
            {
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to get project data, error code:[%d]!", nResult);
              break;
            }

            if (ExportToFile(wsTmp, lpData, u32DataSize))
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Export project data completed.");
            else
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Export project data failed, error code:[%d]!", GetLastError());
          }
        }
        break;
      ...
      case IDC_BTN_IMP:
        if (lpDlgParam->dtType == DT_EDIT)
        {
          ...
          if (GetOpenFilePath(hDlg, wsTmp, MAX_PATH))
          {
            ...
            if (nResult != 0)
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Import data failed, error code:[%d]!", nResult);
            else
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Import data completion.");
      ...
      case IDOK:
      {
        ...
        {
          SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Saving project information failed, error code:[%d]!", nResult);
        }
      }
      break;
